...
write.table(t(c("replicate", "alleleFreqA", "AAmortailityFreq", "BBmortailityFreq", "adultHet", "chickHet_premort", "chickHet_postmort", "hetDiff")), "viaboutput.csv", sep=",", col.names=F, row.names=F, append=F)
  ...
  for(MA in 1:length(mortA)){
    for(MB in 1:length(mortBdiff)){
      ...
      for(r in 1:reps){
        ...
        #apply mortaility to homozygous individuals
        ...
        chicks[chicks[,3]==0, 4] = sample(c(0,1), nrow(chicks[chicks[,3]==0,,drop=FALSE]), prob=c(mortA[MA]-mortBdiff[MB], (1-(mortA[MA]-mortBdiff[MB]))), replace=T)
        ...
        write.table(t(writeout), "viaboutput.csv", sep=",", col.names=F, row.names=F, append=T)
...
####create heatmaps####
alldata = read.table("viaboutput.csv", header=T, sep=",")

minHD = -0.4
maxHD = 0.05
numb  = length(seq(minHD, maxHD, 0.05)) - 1
...
#plot
pdf(file="viabheatmapBB=AA.pdf", width = 7, height = 7)
heatmap(data, Colv=NA, Rowv=NA, breaks=seq(minHD, maxHD, 0.05), col=brewer.pal(numb, "YlOrRd"), scale="none", bg=brewer.pal(numb, "YlOrRd"))
dev.off()
...
#plot
pdf(file="viabheatmapBB=AA-0.15.pdf", width = 7, height = 7)
heatmap(data, Colv=NA, Rowv=NA, breaks=seq(minHD, maxHD, 0.05), col=brewer.pal(numb, "YlOrRd"), scale="none", bg=brewer.pal(numb, "YlOrRd"))
dev.off()
...
#plot
pdf(file="viabheatmapBB=AA-0.30.pdf", width = 7, height = 7)
heatmap(data, Colv=NA, Rowv=NA, breaks=seq(minHD, maxHD, 0.05), col=brewer.pal(numb, "YlOrRd"), scale="none", bg=brewer.pal(numb, "YlOrRd"))
dev.off()
...
#plot
pdf(file="viabheatmapBB=AA-0.45.pdf", width = 7, height = 7)
heatmap(data, Colv=NA, Rowv=NA, breaks=seq(minHD, maxHD, 0.05), col=brewer.pal(numb, "YlOrRd"), scale="none", bg=brewer.pal(numb, "YlOrRd"))
dev.off()
#